                        ...
                        child: TextButton(
                          onPressed: () => showDialog(
                            ...
                            builder: (BuildContext context) {
                              InputValidation _emailValidation2 = InputValidation(isValid: true, message: '');
                              return StatefulBuilder(
                                builder: (context, setState) => AlertDialog(
                                  insetPadding: const EdgeInsets.symmetric(horizontal: 10),
                                  actionsPadding: const EdgeInsets.only(left: 10, top: 0, right: 10, bottom: 10),
                                  ...
                                  actions: [
                                    TextField(
                                      ...
                                      decoration: InputDecoration(
                                        ...
                                        errorText: _emailValidation2.isValid ? null : _emailValidation2.message
                                      )
                                    ),
                                    Padding(
                                      ...
                                      child: ElevatedButton(
                                        onPressed: () async {
                                          if (_emailController2.text.isNotEmpty) {
                                            final provider = Provider.of<EmailSignInProvider>(context, listen: false);
                                            try {
                                              await provider.resetPassword(_emailController2.text);
                                              Navigator.pop(context);
                                            } on FirebaseAuthException catch (e) {
                                              setState(() {
                                                _emailValidation2 = InputValidation(isValid: false, message: tr(e.code));
                                              });
                                            }
                                          }
                                        },
                ...
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    onPressed: () async {
                      ...
                      if (_emailValidation.isValid && _passwordValidation.isValid) {
                        try {
                          final provider = Provider.of<EmailSignInProvider>(context, listen: false);
                          ...
                          } else if (e.code == "weak-password" || e.code == "wrong-password") {
                    ...
                    label: Text('sign_in', style: TextStyle(color: _fontColor)).tr(),
                ...
                SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    onPressed: () async {
                      ...
                      if (_user != null) {
                        try {
                          await UserModel.register(_user.email!, "", _user.displayName!, "1");
                          final userModel = await UserModel.getUserByEmail(_user.email);
                    ...
                    label: Text('sign_in_with_google',style: TextStyle(color: _fontColor)).tr(),
                    icon: FaIcon(FontAwesomeIcons.google, color: _fontColor),
                    style: OutlinedButton.styleFrom(padding: const EdgeInsets.symmetric(vertical: 10)),
                  ),
                ),
              ],
